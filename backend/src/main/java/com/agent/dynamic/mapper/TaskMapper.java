...
public interface TaskMapper {
    ...
    @Insert("INSERT INTO agent_task (name, description, status, result, error, skill_id, params, error_msg, created_at, updated_at) " +
            "VALUES (#{name}, #{description}, #{status}, #{result}, #{error}, #{skillId}, #{params}, #{errorMsg}, #{createdAt}, #{updatedAt})")
    int insert(Task task);

    @Update("UPDATE agent_task SET name=#{name}, description=#{description}, status=#{status}, result=#{result}, " +
