...
13. **必须** 保持单一职责，一个函数只做一件事
14. **必须** 变量/函数命名具有描述性，见名知意
15. **必须** 关键逻辑添加必要注释（WHY，非 WHAT）
16. **必须** 遵循现有代码风格和架构模式
17. **必须** 确保修改后不影响现有功能（回归测试）
18. **必须** 验证编译/运行无错误后才能提交
19. **必须** 不提交敏感信息（密码、Token、密钥等）
20. **必须** 移除调试代码（console.log、print 等）

### Git 规范

21. **禁止** 使用 `git push --force`
22. **禁止** 直接 commit 到 main/master 分支
23. **禁止** commit 包含未完成的代码或被注释的代码
24. **推荐** 使用 feature branch 工作流
25. **推荐** 频繁小提交，每个 commit 功能独立
26. **推荐** commit message 使用 [Conventional Commits](https://www.conventionalcommits.org/) 格式
27. **必须** commit 前检查 `git diff`，确认变更内容正确

### 沟通规范

28. **必须** 在用户打断时立即停止当前工作并响应
29. **必须** 遇到问题及时升级，不要隐藏或搁置问题
30. **必须** 主动告知用户进度和阻塞情况
31. **必须** 验证结果必须附带证据（截图、日志、命令输出）
32. **必须** 对不确定性诚实，不编造未验证的结论

### 内存/持久化规范

33. **必须** 重要上下文记录到 MEMORY.md
34. **必须** 用户明确要求时才保存记忆
35. **禁止** 保存代码模式、架构等可从代码库推导的信息
36. **禁止** 保存临时状态或会话内临时信息
